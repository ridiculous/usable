...
describe Usable do
  ...
  describe ':persisted option' do
    ...
    context 'containing true value' do
      ...
      it 'sets uses the default directory' do
    ...
    context 'containing a options hash' do
      let(:config_dir) { File.expand_path('../..', __FILE__) }

      before do
        subject.usable mod, persisted: { dir: config_dir }
      end

      it 'sets uses the custom directory' do
        subject.usable mod, persisted: true
        subject.new.random = 101
        expect(subject.new._config_file).to match %r|usable/spec/testclass\.yml|
      end
